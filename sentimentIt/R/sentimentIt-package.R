...
#' \itemize{
#' \item The wrapper function will take over your R session, so you'll to start a new session to continue using R or run this from a terminal. If running a Linux/Unix environment the software \code{screen} can allow running commands from the terminal and will continue running even if the terminal window closes.
