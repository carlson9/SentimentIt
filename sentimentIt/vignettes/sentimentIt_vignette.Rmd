...
At this point, the data with unique IDs should be stored both in your R workspace and in the filepath specified in `readText()`. The next step is to create unique batch ID numbers. Each batch ID number will be associated with a "batch",
...
-   `task_setting_id`: The ID number of the HIT settings you would like to use for these batches of comparisons.

-   `num_batches`: The number of separate batches in which you would like to send the comparisons to Mechanical Turk. Default to 1. 
