...
At this point, the data with unique ID numbers should be stored both in your R workspace and in the filepath specified in `readText()`. The next step is to randomly select pairs of ID numbers for objects to be compared. This is done using the `createPairwise()` function. This function will return a matrix

The following example executes the `createPairwise()` function using the output from the `readText()` function that was stored in the object `reviews_with_ids`. In this case, the data had two columns before the ID numbers were added, so `reviews_with_ids` contains three columns. The third column contains the ID numbers, so this must be specified in the `ids` input.

```{r, eval = FALSE}

review_ids_pairwise <- createPairwise(ids = reviews_with_ids[,3],
                                      number_per = 10,
                                      pairwise_path = "/Users/johndoe/Desktop/review_ids_pairwise.Rdata")
```

`createPairwise()` accepts the following arguments:

-   `ids`: Integer vector, the ID numbers of the texts you want to use.

-   `number_per`: Integer, how many documents per batch to be compared.
